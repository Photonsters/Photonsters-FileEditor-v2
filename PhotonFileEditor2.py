...
# TODO:
#   Check PhotonFile.append for correct layerheight calcs
#   AA on Photon Files https://www.facebook.com/groups/AnycubicPhoton/permalink/1621973077947328/
#   Deactivate PhotonSFile - AC dropped support for PhotonS files with new FW in favor of Pws files
#       General props won't fill on PhotonSFile
#       previmage will not allign correctly on photonS (and also photon file)
#       Cython layer image encode/decode in PhotonSFile
...
def mnLoad(filename=None):
    ...
    global fileLib
    if filename==None:
        filename =  tkinter.filedialog.askopenfilename(initialdir = ".",title = "Select file",filetypes = (("photon files","*.photon"),("photon S files","*.photons"),("all files","*.*")))
    ...
    if filetype=='.photon': 
        ...
        fileLib=PhotonFile
    elif filetype=='.photons': 
        pf=PhotonSFile.PhotonSFile()
        fileLib=PhotonSFile
    else:
        root.option_add('*Dialog.msg.font', 'Helvetica 10')    
        messagebox.showinfo("Not supported...",f"Files of type {filetype} are not supported!")
        return        
    
    # Create property widgets on tab1 for selected file type.
    createPropWidgets()
    tab1.update_idletasks()
    root.after(600,resized) # delay will get layer view on tab1 to display correctly
    #resized()
...
def createMenuToolbar():
    ...
    tbEdit.add_separator()
    tbEdit.add_command(os.path.join(installpath,"resources/replace_all.png"),"Repl. *",mnReplaceStack)
...
def editPhotonFileProp(widget,catstr,prop,action):
    ...
    if action in ("enter","tab","mouseleave"):
        newVal=widget.get()
        try:
            if bType==FileLib.tpFloat:newVal=round(float(newVal),FileLib.nrFloatDigits)
            if bType==FileLib.tpInt:newVal=int(newVal)
        except ValueError:
            root.option_add('*Dialog.msg.font', 'Helvetica 10')    
            messagebox.showerror("Invalid entry","Non numeric value in field. Please correct and try again.")
            newVal=FileLib.convBytes(cat[bTitle],bType) 
            return False
    elif action=="escape":
        newVal=FileLib.convBytes(cat[bTitle],bType) 
    ...
    newBytes=FileLib.convVal(newVal)  
    if cat==pf.LayerDefs:    
...
def createPropWidgets():
    ...
    entries_general.clear()
    for prop in fileLib.pfStruct_Header:
        entries_general.append(addPropWidgets("Header",prop))

    addPropWidgets(None,'')
    addPropWidgets(None,'Previews')
    entries_previews.clear()
    for prop in fileLib.pfStruct_Previews:
        entries_previews.append(addPropWidgets("Previews",prop))

    addPropWidgets(None,'')
    addPropWidgets(None,'LayerDefs')
    entries_layerdefs.clear()
    for prop in fileLib.pfStruct_LayerDef:
...
def fillProps(group:str='all'):
    ...
    def setPropVal(p,val,entry):
        ...
        print (">>>",p)
        val=fileLib.convBytes(val,bType)                       
