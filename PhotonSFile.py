...
def bytes_to_int(inbytes:bytes):
    """ Converts list or array of bytes to an int. """
    return (struct.unpack('>i',inbytes)[0])  # big endian

def bytes_to_float(inbytes:bytes):
    """ Converts list or array of bytes to an float. """
    result = 0
    if len(inbytes)==4: return (struct.unpack('>f', inbytes)[0])   # big endian
    if len(inbytes)==8: return (struct.unpack('>d', inbytes)[0])   # big endian    
    raise Exception("Expected a float packed in 4 or 8 bytes!")
...
def float_to_bytes(floatVal:float,nrBytes=8):
    if nrBytes==4: return (struct.pack('>f', floatVal))
    if nrBytes==8: return (struct.pack('>d', floatVal))
